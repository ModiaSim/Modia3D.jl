...
vmat1 = Modia3D.Material(color="LightBlue" , transparency=0.3)    # material of SolidFileMesh
vmat2 = Modia3D.Material(color="Red" , transparency=0.0) # material of solidThing
...
@assembly YouBot begin
    ...
    table              = Table(world)
    thing              = Object3D(world, Solid(SolidCylinder(diameterThing,heigthThing) , "DryWood", vmat2; contactMaterial="DryWood"), r=[xPosTable-tableX/2+diameterThing/2, 0.0, heigthLeg+tableZ/2+heigthThing/2], visualizeFrame=false, fixed=true)
    base               = Base(world)
