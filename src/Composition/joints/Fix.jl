...
struct Fix{F}
    function Fix{F}(; obj1::Object3D{F},
                ...
                rotation::AbstractVector    = Modia3D.ZeroVector3D(F)) where {F}
        ...
        if obj1 === parent
            ...
            if rot == Modia3D.ZeroVector3D(F)
                obj2.joint     = FixedTranslationJoint{F}()
                obj2.jointKind = FixTranslationKind
                obj2.R_rel     = Frames.NullRotation(F)
                obj2.R_abs     = obj1.R_abs
                obj2.r_abs     = obj1.r_abs + r_rel
            else
                obj2.joint     = FixedJoint{F}()
            ...
            if rot == Modia3D.ZeroVector3D(F)
                obj1.joint     = FixedTranslationJoint{F}()
                obj1.jointKind = FixTranslationKind
                obj1.R_rel     = Frames.NullRotation(F)
                obj1.R_abs     = obj2.R_abs
                obj1.r_rel     = -r_rel
            else
                obj1.joint     = FixedJoint{F}()
