...
Return relative SMatrix `R` from frame `frameOrig` into frame `frameMeas`.

If `frameOrig` is omitted `R` represents the absolute rotation of `frameMeas`.
"""
function measFrameRotation(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing) where {F}
...
Return relative position SVector{3,F} `r` from frame `frameOrig` to frame `frameMeas` resolved in frame `frameCoord`.
...
"""
function measFramePosition(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing, frameCoord::Union{Object3D{F}, Nothing}=nothing) where {F}
...
"""
function measFrameDistance(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing) where {F}
...
Return relative rotational velocity SVector{3,F} `w` from frame `frameOrig` to frame `frameMeas` resolved in frame `frameCoord`.
...
"""
function measFrameRotVelocity(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing, frameCoord::Union{Object3D{F}, Nothing}=nothing) where {F}
...
Return relative translational velocity SVector{3,F} `v` from frame `frameOrig` to frame `frameMeas` resolved in frame `frameCoord` and observed in frame `frameObsrv`.
...
"""
function measFrameTransVelocity(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing, frameCoord::Union{Object3D{F}, Nothing}=nothing, frameObsrv::Union{Object3D{F}, Nothing}=nothing) where {F}
...
"""
function measFrameDistVelocity(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing) where {F}
...
Return relative rotational acceleration SVector{3,F} `wd` from frame `frameOrig` to frame `frameMeas` resolved in frame `frameCoord` and observed in frame `frameObsrv`.
...
"""
function measFrameRotAcceleration(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing, frameCoord::Union{Object3D{F}, Nothing}=nothing, frameObsrv::Union{Object3D{F}, Nothing}=nothing) where {F}
...
Return relative translational acceleration SVector{3,F} `a` from frame `frameOrig` to frame `frameMeas` resolved in frame `frameCoord` and observed in frame `frameObsrv`.
...
"""
function measFrameTransAcceleration(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing, frameCoord::Union{Object3D{F}, Nothing}=nothing, frameObsrv::Union{Object3D{F}, Nothing}=nothing) where {F}
...
"""
function measFrameDistAcceleration(frameMeas::Object3D{F}; frameOrig::Union{Object3D{F}, Nothing}=nothing) where {F}
    # (n * v)d = n*vd + nd*v
    #          = n*a + [r/d]d*v
    #          = n*a + [(v*d - r*dd)/(d*d)]*v
