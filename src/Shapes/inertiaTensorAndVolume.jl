...
using LinearAlgebra
EYE3(::Type{F}) where {F} = Matrix(F(1.0)I,3,3)

InertiaMatrix(::Type{F}, matrix) where {F}  = SMatrix{3,3,F,9}(matrix)
...
volume(shape::Sphere{F}) where {F} = F(4/3*pi*(shape.diameter/2)^3)
volume(shape::Ellipsoid{F}) where{F} = F(4/3*pi*shape.lengthX/2*shape.lengthY/2*shape.lengthZ/2)
volume(shape::Cone{F}) where {F} = F(pi/12*shape.length*(shape.diameter^2 + shape.diameter*shape.topDiameter + shape.topDiameter^2))

function volume(shape::Capsule{F}) where {F}
...
"""
@inline centroid(shape::Sphere{F}) where {F} = Modia3D.ZeroVector3D(F)
@inline centroid(shape::Ellipsoid{F}) where {F} = Modia3D.ZeroVector3D(F)
@inline centroid(shape::Box{F}) where {F} = Modia3D.ZeroVector3D(F)
@inline centroid(shape::Cylinder{F}) where {F} = Modia3D.ZeroVector3D(F)
@inline centroid(shape::Capsule{F}) where {F} = Modia3D.ZeroVector3D(F)
@inline centroid(shape::Beam{F}) where {F} = Modia3D.ZeroVector3D(F)

@inline centHeight(shape::Cone{F}) where {F} = F(shape.length/4*(shape.diameter^2 + 2*shape.diameter*shape.topDiameter + 3*shape.topDiameter^2)/(shape.diameter^2 + shape.diameter*shape.topDiameter + shape.topDiameter^2))  # https://mathworld.wolfram.com/ConicalFrustum.html
@inline function centroid(shape::Cone{F}) where {F}
    if shape.axis == 1
