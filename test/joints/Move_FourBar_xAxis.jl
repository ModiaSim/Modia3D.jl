...
@assembly Bar(;Lx = 0.1, Ly=Lx/5, Lz=5*Ly) begin
...
@assembly Fourbar(;Lx = 0.1, Ly=Lx/5, Lz=Ly, groundWidth=Lx, groundHeight=0.1*Lx) begin
   ...
   frame1 = Modia3D.Object3D(world ; r=[groundWidth/2, 0.0, 0.5*Lx])
   frame2 = Modia3D.Object3D(frame1; r=[0.0, 0.0, Lx])
...
#                                              R = ModiaMath.rot_nxy([-1,0,0], [0,0,1]))
   bar1   = Bar(Lx=Lx, Ly=Ly, Lz=5*Lz)
   bar2   = Bar(Lx=Lx, Ly=Ly, Lz=5*Lz)
   bar3   = Bar(Lx=Lx, Ly=Ly, Lz=5*Lz)

   rev1   = Modia3D.Revolute(frame1     , bar1.frame1; axis = 1, phi_start =  pi/2)
   rev2   = Modia3D.Revolute(bar1.frame2, bar2.frame1; axis = 1, phi_start = -pi/2)
   rev3   = Modia3D.Revolute(frame2     , bar3.frame1; axis = 1, phi_start =  pi/2)
   rev4   = Modia3D.Revolute(bar3.frame2, bar2.frame2; axis = 1, phi_start = -pi/2)          # phi_start = -pi/2
end
...
moveModel = Move(sceneOptions=Modia3D.SceneOptions(gravityField=gravField,visualizeFrames=false, defaultFrameLength=0.3))

model  = Modia3D.SimulationModel( moveModel, analysis=ModiaMath.KinematicAnalysis )
result = ModiaMath.simulate!(model, stopTime=3.0, log=false)

println("... success of Move_FourBar_xAxis.jl!")
end
