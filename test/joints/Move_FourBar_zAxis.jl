...
@assembly Fourbar(;Lx = 0.1, Ly=Lx/5, Lz=Ly, groundWidth=Lx, groundHeight=0.1*Lx) begin
   ...
   rev1   = Modia3D.Revolute(frame1     , bar1.frame1; axis = 3, phi_start =  pi/2)
   rev2   = Modia3D.Revolute(bar1.frame2, bar2.frame1; axis = 3, phi_start = -pi/2)
   rev3   = Modia3D.Revolute(frame2     , bar3.frame1; axis = 3, phi_start =  pi/2)
   rev4   = Modia3D.Revolute(bar3.frame2, bar2.frame2; axis = 3, phi_start = -pi/2)          # phi_start = -pi/2
end
...
println("... success of Move_FourBar_zAxis.jl!")
end
