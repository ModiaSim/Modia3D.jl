...
@forceElement Damper(; d=1.0) begin
    pot   = ModiaMath.RealScalar("pot",   causality=ModiaMath.Input,  numericType=ModiaMath.WR)
    flow = ModiaMath.RealScalar("flow", causality=ModiaMath.Output, numericType=ModiaMath.WR)
end
function Modia3D.computeTorque(damper::Damper, sim::ModiaMath.SimulationState)
  damper.flow.value = -damper.d*damper.pot.value
end
...
#=
   body2   = Modia3D.Object3D(Modia3D.Solid(Modia3D.SolidBeam(Lx,Ly,Lz), Modia3D.MassProperties(m=m), vmat1))
   frame2 = Modia3D.Object3D(body2; r=[-Lx/2, 0.0, 0.0])
   cyl2    = Modia3D.Object3D(frame2,Modia3D.Cylinder(Ly/2,1.2*Ly; material=vmat2))
=#

   # Connect pendulum to world
   rev        = Modia3D.Revolute(world, frame1)
   #rev2        = Modia3D.Revolute(world, frame2)

   d     = Damper(d=0.2)
   damper = Modia3D.AdaptorForceElementToFlange(w=d.pot, tau=d.flow)
...
println("... success of Simulate_ControllerDamper.jl!")
end
