...
function robotProgram(robotActions)
    ...
    ptpJointSpace(robotActions, [
            pi   pi/4   pi/4      1.057  0.0  diameter+0.01;
            ])

    ActionAttach(robotActions, "sphereLock", "youbot1.gripper.gripperLock", waitingPeriod=0.1)

    ptpJointSpace(robotActions, [
            pi   0.0    pi/2      0.0    0.0  diameter-0.002;
            ])

    ActionWait(robotActions, 0.2)
    ActionRelease(robotActions, "sphereLock", waitingPeriod=0.1)
    ...
    ptpJointSpace(robotActions, [
            ...
            pi   pi/4   pi/4      1.057  0.0  diameter-0.002;
            ])

    ActionReleaseAndAttach(robotActions, "sphereLock", "youbot1.base.plateLock", waitingPeriod=0.1)
    ...
    ptpJointSpace(robotActions, [
            pi   0.0    0.0      0.0    0.0  diameter-0.002;
            ])
    return nothing
end
...
Base = Model(
    ...
    plate = Object3D(parent=:base_frame, translation=[-0.159, 0, 0.046],
        feature=Solid(shape=FileMesh(filename = plate_obj), massProperties=MassPropertiesFromShapeAndMass(mass=2.397), contactMaterial="DryWood", collision=false)),
    ...
    plate_box = Object3D(parent=:base_frame, translation=[-0.16, 0, 0.0702],# 0.06514],
        feature=Solid(shape=Box(lengthX=0.22, lengthY=0.22, lengthZ=0.01), massProperties=MassPropertiesFromShapeAndMass(mass=1.0e-9), visualMaterial=vmatInvisible, contactMaterial="DryWood", collision=true)),
...
MyGripper = Model(
    ...
    gripper_left_finger = Object3D(parent=:gripper_left_finger_a, translation=[0, 0.0082, 0],
        feature=Solid(shape= FileMesh(filename = gripper_left_finger_obj), massProperties=MassPropertiesFromShapeAndMass(mass=0.010), contactMaterial="DryWood", collision=false)),
    ...
    gripper_right_finger = Object3D(parent=:gripper_right_finger_a, translation=[0, -0.0082, 0],
        feature = Solid(shape = FileMesh(filename = gripper_right_finger_obj), massProperties=MassPropertiesFromShapeAndMass(mass=0.010), contactMaterial="DryWood", collision=false)),
