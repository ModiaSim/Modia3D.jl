...
YouBot = Model(
    ...
    rev2 = RevoluteWithFlange(obj1 = :(link2.obj1), obj2 = :(link2.body),
        axis=axisLink, phi = Var(init = getRefPathInitPosition(referencePath1, 2)), w=Var(init=0.0)),
    rev3 = RevoluteWithFlange(obj1 = :(link3.obj1), obj2 = :(link3.body),
        axis=axisLink, phi = Var(init = getRefPathInitPosition(referencePath1, 3)), w=Var(init=0.0)),
    rev4 = RevoluteWithFlange(obj1 = :(link4.obj1), obj2 = :(link4.body),
        axis=axisLink, phi = Var(init = getRefPathInitPosition(referencePath1, 4)), w=Var(init=0.0)),
    rev5 = RevoluteWithFlange(obj1 = :(link5.obj1), obj2 = :(link5.body),
        axis=axisLink, phi = Var(init = getRefPathInitPosition(referencePath1, 5)), w=Var(init=0.0)),

    prism = PrismaticWithFlange(obj1 = :(gripper.gripper_right_finger_a), obj2 = :(gripper.gripper_left_finger_a),
        axis=axisGripper, s = Var(init = getRefPathInitPosition(referencePath1, 6)) ),
    ...
    equations=:[
        ...
        servo6.refLoadPos   = getRefPathPosition(refPath, 6)
    ],
...
if simplifiedContact
    if Sys.iswindows()
        requiredFinalStates = [-0.0028856253939749917, -0.005097068228759477, 0.3849919434165094, 5.554087798472783e-10, -7.879303496876852e-11, 5.729969588053106e-11, 0.8584445460827014, 1.310275900446975, 8.583198313357519, -4.898396063260283e-10, -2.0401148594086654e-8, -1.1701909106457875e-9, -8.485474873901428e-7, 8.489571559477563e-7, -1.0482213335948063e-5, 1.0483564203621134e-5, -2.386414452809728e-5, 2.386669736569619e-5, -1.4666089477469257e-5, 1.4668439130402333e-5, 1.494345371574345e-7, -1.4944932950998825e-7, -0.006483120795651056, 0.16151976808500465, -0.11771385719683074, -0.051954648224616116, 0.0005198306066959106, -0.01474456635466837, 0.029699159362371413, 0.0003007732575634537]
    else
        requiredFinalStates = [-0.002876745116260849, -0.005097069778674079, 0.3849919438498895, -2.0425691672207105e-11, 1.0417080704922435e-11, -1.4075905125780334e-11, 0.6593656277065468, 1.2469720343755262, 8.790929168495701, 9.600683802328597e-11, 7.90922863241017e-10, -2.122201791290797e-10, -8.485323580937709e-7, 8.489443962226158e-7, -1.0482232651274197e-5, 1.0483595907165604e-5, -2.386430331915021e-5, 2.387225397756575e-5, -1.466612165661964e-5, 1.4668493847153219e-5, 1.494350659637983e-7, -1.4945000593953598e-7, -0.006483005039691437, 0.16151962094686495, -0.11771410145310676, -0.05195475965361245, 0.000519832443564457, -0.014744487892196512, 0.02969916077010009, 0.00030077821507525944]
    end
else
