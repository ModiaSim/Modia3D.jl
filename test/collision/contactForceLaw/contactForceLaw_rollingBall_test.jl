module contactForceLaw_rollingBall_test
...
cmatTable = Modia3D.ElasticContactMaterial2("BilliardTable" ) #  , cor=0.0, mu_k = 0.0, mu_r = 0.0)
cmatBall = Modia3D.ElasticContactMaterial2("BilliardBall" ) # , cor=0.0, mu_k = 0.0, mu_r = 0.0)
...
@assembly Table(world) begin
  ...
  withBox = Modia3D.Solid(Modia3D.SolidBox(LxGround, LyBox, LzBox, rsmall=0.0) , "BilliardTable", vmatSolids; contactMaterial = cmatTable)
...
result = ModiaMath.simulate!(model; stopTime=3.0, tolerance=1e-8,interval=0.001, log=false)

ModiaMath.plot(result, ["ball.sphere.r[3]", "ball.sphere.v[3]", "ball.sphere.w[3]"])
...
println("... success of contactForceLaw_rollingBall_test.jl!")
end
