...
const animationFile = nothing #string("Mobile", depthMax, ".json")
...
mobile = @instantiateModel(buildModia3D(Mobile), evaluateParameters=false, unitless=true, log=false, logDetails=false, logModel=false, logStateSelection=false, logCode=false, logExecution=true, logTiming=false)
include("mobileGetDerivatives.jl")
mobile.getDerivatives! = mobileGetDerivatives2
            @time ModiaLang.invokelatest_getDerivatives_without_der_x!(mobile.x_start, mobile, 0.0)
            @time ModiaLang.invokelatest_getDerivatives_without_der_x!(mobile.x_start, mobile, 0.0)   
...
simulate!(mobile, stopTime=stopTime, tolerance=tolerance, log=true, logTiming=true, requiredFinalStates=requiredFinalStates)
