...

{
    ...
    "title": "Version 0.2.1",
    "category": "section",
    "text": "Adapted to Julia 0.7 and 1.0 (including using new package manager via Project.toml, Manifest.toml files).Travis Continuous Integration added.Wrong UUID of Modia3D and referenced ModiaMath corrected (did not correspond to the UUID in Julias METADATA).PyPlot was removed from the REQUIRE and Project.toml files and code was added, so that PyPlot is automatically imported in ModiaMath if it is available in the current environment of the user. The benefit is that Modia3D and ModiaMath can be used, even if PyPlot is not installed. This is especially useful for ContinuousIntegration, because automatic installation of PyPlot often fails. Inspect the wiki page Installing PyPlot in a robust way to install PyPlot in a robust way.All extra packages used in examples and tests are now referenced via Modia3D (for example using Modia3D.StaticArrays instead of using StaticArrays). The benefit is that all examples and tests can be directly executed with include (for example: import Modia3D; include(Modia3D.path/examples/dynamics/Simulate_Pendulum.jl)) provided Modia3D is in the current environment. Previously, it was assumed that these extra packages are present in the users environment and an error occured, if this was not the case.Dependent packages updated to their newest versions. Especially, warnings from Unitful do no longer occur, due to the update to version 0.12.0."
},

{
    ...
    "title": "Version 0.2.0",
    "category": "section",
    "text": "First public release (for Julia 0.6.4)"
},
...

{
    ...
    "title": "Installation",
    "category": "section",
    "text": "Modia3D is registered in METADATA.jl and can be installed in the following way (Julia >= 1.0 is required):julia> ]add Modia3D\r\n        add ModiaMath  # in order to use simulate!(..) and plot(..)\r\n        add PyPlot     # in order that plots are shownModia3D uses PyPlot for plotting. If PyPlot is not available in your current Julia environment an information message is printed and all plot(..) calls are ignored.In order that plot windows are displayed, you need to add PyPlot to your current environment via ]add PyPlot. Often this automatic installation fails and it is recommended to follow the instructions Installing PyPlot in a robust way.Modia3D visualizes the movement of 3D objects with a renderer. Currently, the (free) community or the (commercial) professional version of the DLR Visualization library are supported. To install the free version for Windows or for Linux perform the following steps:Go to https://visualization.ltx.de/, provide your contact information and click on Request download for Community Edition. Afterwards, you get a link to download the library and you need to unzip the file.In your Julia startup file (HOME/.julia/config/startup.jl), include the environment variable ENV[\"DLR_VISUALIZATION\"] = \"<path-to-library>/Visualization/Extras/SimVis\". Make sure that the SimVis executable under this directory has execution rights. For example in Linux with command: chmod +x <path-to-library>/Visualization/Extras/SimVisStart Julia and run one of the examples, for example include(\"$(Modia3D.path)/examples/dynamics/Simulate_DoublePendulumWithDampers.jl\")If Modia3D cannot use one of the renderers above, it will continue with renderer NoRenderer that is animation is switched off."
},
...
{
    "location": "lib/Composition.html#Modia3D.Composition.Prismatic-Tuple{Object3D,Object3D}",
    "page": "Composition",
...
{
    "location": "lib/Composition.html#Modia3D.Composition.Revolute-Tuple{Object3D,Object3D}",
    "page": "Composition",
...
{
    "location": "lib/Composition.html#Modia3D.Composition.distance-Tuple{Object3D,Object3D}",
    "page": "Composition",
